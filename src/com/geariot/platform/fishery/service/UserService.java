...
public class UserService {
    ...
    public Map<String, Object> relationDetail(String relation) {
        ...
        switch (type) {
            case "WX":
                return WXUserDetail(Integer.parseInt(relation.substring(2)));
            case "CO":
                return CompanyDetail(Integer.parseInt(relation.substring(2)));
            default:
    ...
    public String getPublicOpenId(String openId) {
        ...
        if (wxuserList != null && !wxuserList.isEmpty()) {
            for (WXUser wxuser : wxuserList) {
                if (wxuser.getUnionid() != null && !"".equals(wxuser.getUnionid())) {
        ...
        if (unionId != null && !"".equals(unionId)) {//在小程序用户表中unionId不为空
