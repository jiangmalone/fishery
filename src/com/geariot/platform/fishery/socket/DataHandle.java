...
public class DataHandle {
	 ...
	 /*private static Map<String, SocketChannel> clientMap = new HashMap<String, SocketChannel>();// id SocketChannel
	    private static List<MsgStruct> msg_list = new ArrayList<MsgStruct>();
	 			...
	 			try
	 			{
	 				switch(order)
	 				{
	 					...
	 					case 7:
	 						/*byte power7 = data[7];
	 						byte check7 = data[8];
	 						String suffix7 = CommonUtils.printHexStringMerge(data,9,4);
	 						Dao_AeratorStatus dao_AeratorStatus1 = new Dao_AeratorStatus();
	 						row_count = dao_AeratorStatus1.insertOne(String.valueOf(id), way,power7, "2");
	 						Date now1 = new Date();
	 						for(int i=0;i<msg_list.size();i++)
	 						{
	 							if((now1.getTime()-msg_list.get(i).getStart_time())/1000>20)
	 							{
	 								msg_list.get(i).getChannel().close();
	 								msg_list.remove(i);
	 							}
	 						}
	 						Iterator<MsgStruct> it1 = msg_list.iterator();
	 						while(it1.hasNext())
	 						{
	 							MsgStruct one = it1.next();
	 							if((now1.getTime()-one.getStart_time())/1000>20)
	 						    {
	 							...
	 							{
	 								byte[] device_sn_byte = CommonUtils.getByteArray(id);
	 								if(one.getId()==id&one.getWay()==way&one.getOrder()==2)
	 								{
	 									response = new byte[9];
	 									response[0]=(byte)0x5B;
	 									response[1]=(byte)0xB5;
	 									response[2]=(byte)0x01;
	 									response[3]=device_sn_byte[0];
	 									response[4]=device_sn_byte[1];
	 									response[5]=device_sn_byte[2];
	 									response[6]=(byte)way;
	 									if(row_count==1)//0 success 1 offline 2 fail
	 									{
	 										response[7]= (byte)0x00; 
	 									}
	 									else
	 									{
	 										response[7]= (byte)0x02;
	 									}
	 									response[8] = power7;
	 									ByteBuffer outBuffer = ByteBuffer.wrap(response);
	 									one.getChannel().write(outBuffer);
	 									one.getChannel().close();
	 									it1.remove();
	 									break;
	 								}
	 						...
	 						System.out.println("server set status success !!!");*/
	 						break;
	 					case 8:
	 						/*byte check8 = data[7];
...
//	 						row_count = dao_AeratorStatus1.insertOne(String.valueOf(id), way,power7, "2");
	 						...
	 						Iterator<MsgStruct> it2 = msg_list.iterator();
	 						while(it2.hasNext())
	 						{
	 							MsgStruct one = it2.next();
	 							if((now2.getTime()-one.getStart_time())/1000>20)
	 						    {
	 								one.getChannel().close();
	 						      it2.remove();
	 						    }
	 							...
	 							{
	 								byte[] device_sn_byte = CommonUtils.getByteArray(id);
	 								if(one.getId()==id&one.getWay()==way&one.getOrder()==4)
	 								{
	 									response = new byte[8];
	 									response[0]=(byte)0x5B;
	 									response[1]=(byte)0xB5;
	 						...
	 						System.out.println("server set auto success !!!");	*/					
	 						break;
	 					case 9:
	 					...
	 					case 12: //0C
	 						...
	 						Iterator<MsgStruct> it12 = msg_list.iterator();
	 						while(it12.hasNext())
	 						{
	 							MsgStruct one = it12.next();
	 							if((now12.getTime()-one.getStart_time())/1000>20)
	 						    {
	 								one.getChannel().close();
	 						      it12.remove();
	 						    }
	 							...
	 							{
	 								byte[] device_sn_byte = CommonUtils.getByteArray(id);
	 								if(one.getId()==id&one.getWay()==way&one.getOrder()==5)
	 								{
	 									response = new byte[8];
	 									response[0]=(byte)0x5B;
	 									response[1]=(byte)0xB5;
	 									response[2]=(byte)0x05;
	 									response[3]=device_sn_byte[0];
	 									response[4]=device_sn_byte[1];
	 									response[5]=device_sn_byte[2];
	 									response[6]=(byte)way;
	 					...
	 					case 13://0D
	 						...
	 						Iterator<MsgStruct> it3 = msg_list.iterator();
	 						while(it3.hasNext())
	 						{
	 							MsgStruct one = it3.next();
	 							if((now3.getTime()-one.getStart_time())/1000>20)
	 						    {
	 								one.getChannel().close();
	 						      it3.remove();
	 						    }
	 							...
	 							{
	 								byte[] device_sn_byte = CommonUtils.getByteArray(id);
	 								if(one.getId()==id&one.getWay()==way&one.getOrder()==3)
	 								{
	 									response = new byte[8];
	 									response[0]=(byte)0x5B;
	 									response[1]=(byte)0xB5;
	 									response[2]=(byte)0x03;
	 									response[3]=device_sn_byte[0];
	 									response[4]=device_sn_byte[1];
	 									response[5]=device_sn_byte[2];
	 									response[6]=(byte)way;
	 						...
	 						System.out.println("server set correct success !!!");*/
	 						break;
	 					default:
	 						break;
	 				}
