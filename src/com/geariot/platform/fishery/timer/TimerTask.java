            ...
            if (response.errno == 0) {
                ...
                if (isOnline) {
                    ...
                    if (deviceOld.isOnline()) {
                        ...
                        //如果用户手动登出小程序，会清空其openId和unionId，所以如果查询不到这个人的openId，就不给其推送消息了。
                        String wxUserOpenId = wxUser.getOpenId();
                        logger.debug("用户openId：" + wxUserOpenId);
                        if (wxUserOpenId != null && !"".equals(wxUserOpenId)) {
                            String publicOpenID = userService.getPublicOpenId(wxUserOpenId);
                            logger.debug("用户的publicOpenID：" + publicOpenID);
                            WechatSendMessageUtils.sendWechatAlarmMessages("设备离线", publicOpenID, device.getDevice_sn(), deviceName);
                        } else {
                            logger.debug("用户openId为空，打印wxUser对象：" + wxUser);
                        }

                        //语音推送
                        String wxUserPhone = wxUser.getPhone();
                        if (wxUserPhone != null && !"".equals(wxUserPhone)) {
                            String json = "{\"deviceName\":\"" + deviceName + "\",\"way\":" + "0" + "}";
                            try {
                                logger.debug("准备启用阿里云语音服务");
                                VmsUtils.singleCallByTts(wxUserPhone, "TTS_142385982", json);
